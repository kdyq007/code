...
char *m_HOST ="10.0.128.220";
char *m_USERNAME= "root";
char *m_PASSWORD ="123u123u";
char *m_DATABASE ="eas";
...
*/
	...
	init_MysqlQi(option.mysql_IP,option.mysql_user,option.mysql_pass,option.mysql_db,option.mysql_port);

	if(ConnectDB()==-1)
	{
		s_log(eERROR,"ConnectDB() failed!");
		exit(EXIT_FAILURE);
	}
...
void sql_client_logoff(struct client_info *c)
{
	...
	snprintf(sqlbuf,sizeof(sqlbuf)-1,"UPDATE USER SET duration=%d,status=%s,hash_session=%d WHERE id=%d",duration(),(c->cid.eject==1)?("EJECTED"):("COMPLETE"),c->cid.hash_session,c->cid.mysqlrowid);
	ExecSQL(sqlbuf);
        DisConnectDB();
