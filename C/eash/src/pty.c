...
int main(int argc, char **argv)
{
	...
	char env_tty[BUFSIZ];
	char env_ssh_auth_sock[BUFSIZ];
	char env_ssh_client[BUFSIZ];
	char env_ssh_connection[BUFSIZ];
	char env_ssh_tty[BUFSIZ];
	...
	if(getenv("SSH_AUTH_SOCK"))
		snprintf(env_ssh_auth_sock, sizeof(env_ssh_auth_sock) - 1, "SSH_AUTH_SOCK=%.63s", getenv("SSH_AUTH_SOCK"));
	if(getenv("SSH_CLIENT"))
		snprintf(env_ssh_client, sizeof(env_ssh_client) - 1, "SSH_CLIENT=%.63s", getenv("SSH_CLIENT"));
	if(getenv("SSH_CONNECTION"))
		snprintf(env_ssh_connection, sizeof(env_ssh_connection) - 1, "SSH_CONNECTION=%.63s", getenv("SSH_CONNECTION"));
	if(getenv("SSH_TTY"))
		snprintf(env_ssh_tty, sizeof(env_ssh_tty) - 1, "SSH_TTY=%.63s", getenv("SSH_TTY"));
	...
	env_list[c++] = env_tty;
	env_list[c++] = env_ssh_auth_sock;
	env_list[c++] = env_ssh_client;
	env_list[c++] = env_ssh_connection;
	env_list[c++] = env_ssh_tty;
	...
	if(connected == 0)
	{
		...
		fprintf(stderr, "Connect to Mnet failed!\nPlease contact with sharlockqi@123u.com!\n");
	...
	while(1)
	{
		...
		if(FD_ISSET(STDIN_FILENO, &fds))
		{
			...
			if(memcmp(buf,"\033",1)==0)
			{
				if(memcmp(buf,"\033[A",3)==0||memcmp(buf,"\033[B",3)==0||memcmp(buf,"\033[C",3)==0||memcmp(buf,"\033[D",3)==0)
                       		{
                        	        CtrlKey=1;
                        	}
                        	else
...
void rawmode(void)
{
	...
	newttyattr.c_iflag = BRKINT |IXON | IXANY;
