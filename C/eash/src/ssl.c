	...
	while(client->cid.eject == 0 && !received_shutdown)
	{
		...
		if(SSL_pending(ssl_socket))
		{
			...
			if(client->cid.ready)
			{
				...
				memcpy(&sd,buf,sizeof(buf));	
				sd.data[sd.outlen]='\0';
				h.len=sd.outlen;
				memset(buf, '\0', sizeof(buf));
				memcpy(buf,sd.data,sd.outlen);

				gettimeofday(&h.tv, NULL);

				write_header(client->cid.session, &h);
				fwrite(buf, 1, h.len, client->cid.session);

				if(option.sync)
					fflush(client->cid.session);
				
				//me
				if(sd.inlen<sd.outlen)
				{
					if(sd.data[0]=='\b')
					{
						if(buf_seek_another>0)
						{
							--buf_seek_another;
						}
						tbuf[buf_seek_another]='\0';
						continue;
					}

					if(sd.inlen!=0)
					{
						memcpy(tbuf+buf_seek_another,buf,sd.inlen);
						memset(sqlbuf, '\0', sizeof(sqlbuf));
				        	snprintf(sqlbuf,sizeof(sqlbuf)-1,"insert into data(sessionID,io,sec,usec,len,data)values(%d,1,%d,%d,%d,?);",client->cid.mysqlrowid,h.tv.tv_sec,h.tv.tv_usec,buf_seek_another+sd.inlen);
				        	stmt_mysql(sqlbuf,tbuf,buf_seek_another+sd.inlen);
						memset(tbuf, '\0', sizeof(tbuf));
					}

					memcpy(tbuf,buf+sd.inlen,sd.outlen-sd.inlen);
					memset(sqlbuf, '\0', sizeof(sqlbuf));
			        	snprintf(sqlbuf,sizeof(sqlbuf)-1,"insert into data(sessionID,io,sec,usec,len,data)values(%d,0,%d,%d,%d,?);",client->cid.mysqlrowid,h.tv.tv_sec,h.tv.tv_usec,sd.outlen-sd.inlen);
			        	stmt_mysql(sqlbuf,tbuf,sd.outlen-sd.inlen);

					memset(tbuf, '\0', sizeof(tbuf));
					buf_seek_another=0;
				}
				else if(sd.inlen==sd.outlen)
				{
									memcpy(tbuf+buf_seek_another,buf,sd.inlen);
					buf_seek_another += sd.inlen;
				}
                                else
                                {	
					if(sd.data[0]=='\b')
					{
						snprintf(sqlbuf,sizeof(sqlbuf)-1,"insert into data(sessionID,io,sec,usec,len,data)values(%d,1,%d,%d,%d,?);",client->cid.mysqlrowid,h.tv.tv_sec,h.tv.tv_usec,sd.inlen);
	                                        stmt_mysql(sqlbuf,sd.data,sd.inlen);
					}
					else
					{
						ssl_error("transmission data error!");
                                        	return (-1);
					}
                                }
			}
...
size_t eas_my_read(SSL *ssl, char *ptr)
{
...
int parse_protocol(SSL *ssl_socket, struct client_info *c, const void *line)
{
        ...
        if((n = sscanf(line, "USER\a%63[^\a]\a%i\a%63[^\a]\a%i\a%63[^\a]\a%i\a%63[^\a]\a%i\a%63[^\a]\a%i\a%63[^\a]\a%i\a%100[^\n]", (char *) &original_pw_name, &original_uid, (char *) &original_gr_name, &original_gid, (char *) &real_pw_name, &real_uid, (char *) &real_gr_name, &real_gid, (char *) &effective_pw_name, &effective_uid, (char *) &effective_gr_name, &effective_gid, (char *) &terminal)) == 13)
	{
		...
		*/
		...
		//me
                memset(sqlbuf, '\0', sizeof(sqlbuf));
		snprintf(sqlbuf,sizeof(sqlbuf)-1,"INSERT INTO session (real_uid,real_gid,effective_uid,effective_gid,original_uid,original_gid,port,duration,real_pw_name,real_gr_name,effective_pw_name,effective_gr_name,original_pw_name,original_gr_name,terminal,ip,status,stype,client_ip,pid) VALUES(%d,%d,%d,%d,%d,%d,%d,%d,'%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s',%d);",c->cid.real_uid,c->cid.real_gid,c->cid.effective_uid,c->cid.effective_gid,c->cid.original_uid,c->cid.original_gid,c->cid.port,0,c->cid.real_pw_name,c->cid.real_gr_name,c->cid.effective_pw_name,c->cid.effective_gr_name,c->cid.original_pw_name,c->cid.original_gr_name,c->cid.terminal,c->cid.ip,"R",c->cid.type,c->nodename,getpid());
		ExecSQL(sqlbuf);

		ExecSQL("select last_insert_id();");
		if(StoreResult()==-1)
        	{
                	printf("failed to StoreResult!\n");
			return (-1);
        	}
        	c->cid.mysqlrowid=GetInsertRow();
		memset(sqlbuf, '\0', sizeof(sqlbuf));
                snprintf(sqlbuf,sizeof(sqlbuf)-1,"UPDATE session SET created = now() WHERE id = %d;",c->cid.mysqlrowid);
                ExecSQL(sqlbuf);
		...
		//me
		memset(sqlbuf, '\0', sizeof(sqlbuf));
		snprintf(sqlbuf,sizeof(sqlbuf)-1,"UPDATE session SET file_session='%s' WHERE id=%d;",c->cid.file_session,c->cid.mysqlrowid);
		ExecSQL(sqlbuf);
